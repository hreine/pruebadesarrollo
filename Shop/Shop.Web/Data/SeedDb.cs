namespace Shop.Web.Data
{
    ...
    public class SeedDb
    {
        ...
        public async Task SeedAsync()
        {
            ...
            if (!this.context.ServiceType.Any())
            {
               this.context.ServiceType.Add(new ServiceType { Name = "Repuesto" });
               this.context.ServiceType.Add(new ServiceType { Name = "Mano de Obra" });
                await this.context.SaveChangesAsync();
            }

            if (!this.context.PartMaster.Any())
            {
                this.context.PartMaster.Add(new PartMaster { ServiceTypeId =1,Note="Chumasera BMW",Price=50,MaxPrice=70,MinPrice=45,Quantity=100});
                this.context.PartMaster.Add(new PartMaster { ServiceTypeId = 1, Note = "Aceite champuza", Price = 10, MaxPrice = 15, MinPrice = 9, Quantity = 1500 });
                this.context.PartMaster.Add(new PartMaster { ServiceTypeId = 2, Note = "Limpieza de carburador", Price = 10, MaxPrice = 15, MinPrice = 9, Quantity = 0 });
                this.context.PartMaster.Add(new PartMaster { ServiceTypeId = 2, Note = "Cambio de Aceite", Price = 20, MaxPrice = 30, MinPrice = 29, Quantity = 0});
                this.context.PartMaster.Add(new PartMaster { ServiceTypeId = 2, Note = "Balanceo", Price = 100, MaxPrice = 125, MinPrice = 90, Quantity = 0 });
                await this.context.SaveChangesAsync();
            }


            if (!this.context.DocType.Any())
            {
                await this.AddDocTypeAsync();
            }
            ...
            var user = await this.CheckUserAsync("hreine@gmail.com", "Rafael", "Reines", "Admin");
        ...
        private async Task AddDocTypeAsync()
        {
            this.AddDocType("CC");
            this.AddDocType("NIT");
            await this.context.SaveChangesAsync();
        ...
        private void AddDocType(string docType)
        {            
            var newobj = new DocType
            {
                Name = docType
            };
            this.context.DocType.Add(newobj);
        }
